...
export default async function ClientsPage({
	...
	return (
		<div className="container mx-auto px-6 py-20 max-w-7xl">
					...
					<div className="mb-6 flex justify-center">
						<div className="px-4 py-2 bg-white border border-gray-200 rounded-full text-sm font-medium text-gray-600 inline-flex items-center shadow-sm">
							...
							<AnimatedShinyText shimmerWidth={150}>
								+500 MCP Clients in list
							</AnimatedShinyText>
						</div>
					</div>

					<h1 className="text-5xl md:text-7xl font-bold mb-6">
						<span className="text-transparent bg-gradient-to-r from-blue-500 to-purple-500 bg-clip-text text-gray-900">
							Discover{' '}
						</span>
						<span>
				...
				<ScrollArea className="w-full whitespace-nowrap">
					<div className="flex space-x-2 py-4">
						...
						{categories.map((category, index) => (
							<Link
								key={index}
								href={`/clients?category=${category}${
									searchQuery ? `&search=${searchQuery}` : ''
								}`}
								className={clsx(
									'inline-block',
			...
			<h2 className="text-2xl font-bold mb-6">All MCP Clients</h2>
